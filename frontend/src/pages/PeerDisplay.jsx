...
const PeerDisplay = () => {
  ...
  const fetchTutors = async () => {
    try {
      ...
      const res = await axios.get('http://localhost:5000/api/student/tutors?type=Peer', {
        headers: { 'x-auth-token': token }
      });
      setTutors(res.data);
    } catch (err) {
      console.error(err);
      setError('Failed to load peer tutors.');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <Layout type="student">
      <div className="tutor-display-header">
        <h1 className="tutor-display-title text-warning">Peer Tutors</h1>
        <p className="text-muted mt-2">Top-performing university students ready to help you succeed.</p>
      </div>
      ...
      <div className="tutors-container">
        {loading ? (
          <div className="text-center mt-4">
            ...
            <p className="mt-2 text-muted">Loading peer tutors...</p>
          </div>
        ) : error ? (
          ...
          <div className="text-center p-5 bg-card rounded border">
             ...
             <h4 className="text-muted fw-bold">No Peer Tutors Available</h4>
          ...
          <div className="tutor-grid">
            {tutors.map(tutor => (
              <div key={tutor.id} className="tutor-profile-card">
                <div className="tutor-profile-header peer-header">
              ...
              <form onSubmit={handleSubmitDemo}>
                ...
                {selectedTutor && (
                  <div className="tutor-info-alert">
                    <i className="fas fa-info-circle"></i> Requesting a <strong>Peer</strong> tutor session with <strong>{selectedTutor.name}</strong>.
                  </div>
                )}
                
                <button type="submit" className="btn btn-primary w-100 mt-2" disabled={submitting} style={{backgroundColor: 'var(--warning)'}}>
